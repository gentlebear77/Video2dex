...
class MujocoPointcloudWrapperAdroit(gym.Wrapper):
    ...
    def __init__(self, env, env_name:str, use_point_crop=True):
        # print(super(type(env)).__self_class__)
        super().__init__(env)
        ...
        self.cam_names=ENV_POINT_CLOUD_CONFIG[env_name]['cam_names']
        # point cloud generator
        if env_name.split('_')[0] == 'adroit':
            self.pc_generator = PointCloudGenerator(sim=env.get_mujoco_sim(),
                                                cam_names=ENV_POINT_CLOUD_CONFIG[env_name]['cam_names'])
        else :
            self.pc_generator = PointCloudGenerator(sim=env.sim,
                                                cam_names=ENV_POINT_CLOUD_CONFIG[env_name]['cam_names'])
    ...
    def get_point_cloud(self, use_RGB=True):
        ...
        # save_img_dir = "/home/Diffusion_Policy/3D-Diffusion-Policy/fig"
        save_img_dir = None
    ...
    def step(self, action):
        ...
        if 'adroit' or 'dexmv' in self.env_name: # adroit uses a namedtuple for obs
