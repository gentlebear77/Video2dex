...
class PointCloudGenerator(object):
    ...
    def __init__(self, sim, cam_names:List, img_size=84):
        ...
        for idx in range(len(self.cam_names)):
            # get camera id
            cam_id = self.sim.camera_name2id(self.cam_names[idx])
            fovy = math.radians(self.sim.cam_fovy[cam_id])
    ...
    def generateCroppedPointCloud(self, save_img_dir=None, device_id=0):
        ...
        for cam_i in range(len(self.cam_names)):
            ...
            cam_body_id = self.sim.cam_bodyid[cam_i]
            cam_pos = self.sim.body_pos[cam_body_id]
            c2b_r = rotMatList2NPRotMat(self.sim.cam_mat0[cam_i])
    ...
    def depthimg2Meters(self, depth):
        extent = self.sim.stat.extent
        near = self.sim.vis.map.znear * extent
        far = self.sim.vis.map.zfar * extent
